...
final class PrivacyBlurOverlay {
    ...
    private let fadeInDuration: TimeInterval = 0.30
    ...
    private func makePanel(for screen: NSScreen) -> NSPanel {
        ...
        panel.contentView = makeContentView(frame: localBounds)
    ...
    private func makeContentView(frame: NSRect) -> NSView {
        ...
        tintView.layer?.backgroundColor = NSColor.white.withAlphaComponent(0.055).cgColor

        let dimView = NSView(frame: frame)
        dimView.autoresizingMask = [.width, .height]
        dimView.wantsLayer = true
        dimView.layer?.backgroundColor = NSColor.black.withAlphaComponent(0.22).cgColor
